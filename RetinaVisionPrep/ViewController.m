...
//    self.videoCamera.defaultFPS = 30;
    ...
    // GI
    NSString* filePath7 = @"GI_file";
    NSString* fileRoot7 = [[NSBundle mainBundle] pathForResource:filePath7 ofType:@"txt"];
    NSString* fileContents7 = [NSString stringWithContentsOfFile:fileRoot7 encoding:NSUTF8StringEncoding error:nil];
    //
    //        // array of lines
    NSArray* allLinedStrings7 = [fileContents7 componentsSeparatedByCharactersInSet:[NSCharacterSet newlineCharacterSet]];
    NSString* strsInOneLine7 = [allLinedStrings7 objectAtIndex:0];
    NSArray* singleStr7 = [strsInOneLine7 componentsSeparatedByCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@" "]];
    shape0 = [singleStr7[0] intValue];
    shape1 = [singleStr7[1] intValue];
    
    GI = cv::Mat(shape0,shape1,CV_32F ,0.0);
    for(int i=1;i< (int)([allLinedStrings7 count]-1);i++){
        NSString* elements = allLinedStrings7[i];
        //            NSLog(@"%@", elements);
        NSArray* singleStrs = [elements componentsSeparatedByCharactersInSet: [NSCharacterSet characterSetWithCharactersInString:@"#"]];
        //            NSLog(@"%@",singleStrs);
        for(int j=0;j<(int)[singleStrs count];j++){
            GI.at<float>(i-1,j) = [singleStrs[j] floatValue];
        }
    }
//    print(GI);
